    ...
    <div class="pt-32 pb-24 relative z-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="reveal-on-scroll mb-8" style="transition-delay: 0.1s;">
                 <a href="{{ route('products.index') }}" class="text-xs font-sans uppercase tracking-widest text-forest/60 hover:text-medical transition-colors flex items-center">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
                ...
                <div class="reveal-on-scroll flex flex-col" style="transition-delay: 0.4s;">
                    ...
                    <h1 class="font-serif text-4xl md:text-5xl text-forest tracking-wide mb-4">{{ $product->title }}</h1>
                    
                    @if($product->short_description)
                        <p class="font-sans text-forest/80 text-lg leading-relaxed mb-6">{{ $product->short_description }}</p>
                    @endif

                    <div class="font-sans text-forest/70 text-lg font-light leading-relaxed mb-8 prose prose-p:text-forest/70 max-w-none">
                        {!! $product->description !!}
                    </div>
                    
                    @if($product->clinical_focus)
                    <div class="bg-beige/50 border border-olive/20 rounded-xl p-6 mb-12">
                         <h3 class="font-sans text-xs uppercase tracking-widest text-forest/60 mb-2">Clinical Focus</h3>
                         <p class="font-serif text-forest text-xl">{{ $product->clinical_focus }}</p>
                    </div>
                    @endif
                    ...
                    <div class="w-full" x-data="{ tab: 'benefits' }">
                        ...
                        <div class="flex space-x-8 border-b border-olive/20 mb-8 overflow-x-auto pb-[1px]" role="tablist">
                            ...
                            <button @click="tab = 'benefits'" :class="{ 'active text-medical border-medical': tab === 'benefits', 'text-forest/60 border-transparent hover:text-medical': tab !== 'benefits' }" class="pb-4 text-sm font-sans uppercase tracking-[0.15em] transition-colors border-b whitespace-nowrap" role="tab">Benefits</button>
                            @endif
                            @if(!empty($product->composition))
                            <button @click="tab = 'ingredients'" :class="{ 'active text-medical border-medical': tab === 'ingredients', 'text-forest/60 border-transparent hover:text-medical': tab !== 'ingredients' }" class="pb-4 text-sm font-sans uppercase tracking-[0.15em] transition-colors border-b whitespace-nowrap" role="tab">Ingredients</button>
                            @endif
                            @if(!empty($product->protocol_summary) || !empty($product->protocol_details))
                            <button @click="tab = 'protocol'" :class="{ 'active text-medical border-medical': tab === 'protocol', 'text-forest/60 border-transparent hover:text-medical': tab !== 'protocol' }" class="pb-4 text-sm font-sans uppercase tracking-[0.15em] transition-colors border-b whitespace-nowrap" role="tab">Protocol</button>
                            @endif
                        </div>
                        ...
                        <div class="relative min-h-[200px]">
                            ...
                            <div x-show="tab === 'benefits'" x-transition:enter="transition opacity-0" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="tab-content" role="tabpanel">
                                <ul class="space-y-4">
                                    ...
                                    <li class="flex items-start">
                                        ...
                                        <span class="font-sans font-light text-forest/80 leading-relaxed">{{ $benefit['benefit'] ?? $benefit }}</span>
                                    </li>
                                    @endforeach
                                </ul>
                            </div>
                            ...
                            <div x-show="tab === 'ingredients'" x-transition:enter="transition opacity-0" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="tab-content" role="tabpanel" style="display: none;">
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="bg-white/40 p-4 rounded-lg border border-cream/50">
                                        <span class="block font-serif text-forest mb-1">{{ $composition['name'] ?? '' }}</span>
                                        <span class="text-xs font-sans text-forest/60 uppercase tracking-wider">{{ $composition['description'] ?? '' }}</span>
                                    </div>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div x-show="tab === 'protocol'" x-transition:enter="transition opacity-0" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="tab-content" role="tabpanel" style="display: none;">
                                ...
                                <p class="font-sans font-light text-forest/80 leading-relaxed mb-6">{{ $product->protocol_summary }}</p>
                                @endif
                                ...
                                <div class="border-l border-medical/30 pl-6 space-y-6">
                                    ...
                                    <div>
                                        <h4 class="font-serif text-forest mb-1">{{ $detail['step'] ?? '' }}</h4>
                                        <p class="font-sans text-sm text-forest/70 font-light">{{ $detail['value'] ?? '' }}</p>
                                    </div>
                                    @endforeach
                                </div>
                                @endif
            ...
            <div class="mt-24 reveal-on-scroll border-t border-olive/20 pt-16 group/gallery">
                <div class="flex flex-col md:flex-row md:items-end justify-between mb-12">
                    <div>
                        <h2 class="font-serif text-3xl md:text-4xl text-forest tracking-wide">Gallery</h2>
                    </div>
                </div>
